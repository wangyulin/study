    ...
    private static void printMemoryPoolInfo(){
        ...
        if(pools != null && !pools.isEmpty()){
            for(MemoryPoolMXBean pool : pools){
                ...
                System.out.printf("vm内存区:\n\t名称=%s\n\t所属内存管理者=%s\n\t ObjectName=%s\n\t初始大小(M)=%d\n\t最大(上限)(M)=%d\n\t已用大小(M)=%d\n\t已提交(已申请)(M)=%d\n\t使用率=%d%%%n", pool.getName(), Arrays.deepToString(pool.getMemoryManagerNames()), pool.getObjectName(), pool.getUsage().getInit() / MB, pool.getUsage().getMax() / MB, pool.getUsage().getUsed() / MB, pool.getUsage().getCommitted() / MB, pool.getUsage().getUsed() * 100 / pool.getUsage().getCommitted());
