...
class PersonAddreRedu extends Reducer<IntWritable, Bean, NullWritable, Text> {
	...
	protected void reduce(IntWritable key, Iterable<Bean> values,
			Reducer<IntWritable, Bean, NullWritable, Text>.Context context) throws IOException, InterruptedException {
		Bean Addre = null;
...
class PersonAddrMap extends Mapper<LongWritable, Text, IntWritable, Bean> {
	@Override
	protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, IntWritable, Bean>.Context context)
